...
#ifdef CUSP
   extern void CUSP_pressureCorrectionOperations();
#endif

#ifdef MKLPARDISO
   extern int pardisoSolver_momentum();
   extern int pardisoSolver_deltaP();   
#endif
...
//------------------------------------------------------------------------------
{
   ...
   for (iter=1; iter < nonlinearIterMax; iter++) {   
      ...
      for (phase=0; phase<3; phase++) {
         ...
         #ifdef MKLPARDISO
            pardisoSolver_momentum();  // Non-sym, positive def
         #endif
         ...
         switch (phase) {
            case 0:
               ...
               cout << "   x-momentum is solved." << endl;               
               break;
            case 1:
               for (i=0; i<NN; i++) {
                  v_temp[i] = velVector[i];                
               }   
               cout << "   y-momentum is solved." << endl;  
               break;
            case 2:
               for (i=0; i<NN; i++) {
                  w_temp[i] = velVector[i];                  
               }   
               cout << "   z-momentum is solved." << endl;
               break;
         }
      }
      for (i=0; i<NN; i++) {
