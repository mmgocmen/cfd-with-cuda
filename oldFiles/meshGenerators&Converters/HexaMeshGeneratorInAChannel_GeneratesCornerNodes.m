%**********************************************************************
%                                                                     *
% 3D hexa mesh generator in a channel of size [x0,x1]x[y0,y1]x[z0,z1] *
% It can generate meshes with 8 velocity nodes and 27 velocity nodes. *
% Only the corner nodes are written, others will be calculated        *
% inside the flow solver.                                             *
%                                                                     *
...
%                                                                     *
% The generated mesh is written to a file called ChannelMesh.inp      *
%                                                                     *
%**********************************************************************
%                                                                     *
% Change Log                                                          *
%                                                                     *
% 01/10/2013 : Initial version.                                       *
%                                                                     *
%**********************************************************************

function [] = HexaMeshGeneratorInAChannel()
clc;
clear all;
close all;

disp('****************************************************');
disp('***     3D Hexa Mesh Generator in a Channnel     ***');
disp('****************************************************');
...
NEx = 40;            % Number of elements in x direction. Select an even number for proper clustering.
NEy = 10;
NEz = 10;
clusterCoeff = 0.0;  % Enter 0.0 for no clustering. A positive clustering
                     % will squeeze the nodes towards the inlet.
...
file = fopen(strcat('ChannelMesh.inp'), 'w');
...
fprintf(file, '%d\n', NEz/2*(NEx+1)*(NEy+1) + (NEy/2+1)*(NEx+1));    % Set pressure to zero at the mid-point of the exit.

fprintf(file, '================================================\n');
fprintf(file, 'Monitor point coordinates\n');
fprintf(file, '10.0  0.5  0.5\n');

fclose(file);

fprintf('\nA file named ChannelMesh3D.inp is generated. \n');
